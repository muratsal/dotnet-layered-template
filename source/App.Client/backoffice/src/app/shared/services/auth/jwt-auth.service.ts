...
export class JwtAuthService {
  ...
  public signin(username, password) {
    ...
    return this.http.post(`${environment.apiURL}/Login`, { username, password })
      .pipe(
        map((res: any) => {
          this.setUserAndToken(res.token,{ id:res.id, displayName:res.userName ,permissions:res.permissions }, !!res);
          this.signingIn = false;
          return res;
        }),
        catchError((error) => {
  ...
  */
  public checkTokenIsValid() {
    ...
    */
    ...
    return this.http.get(`${environment.apiURL}/Login/Profile`)
      .pipe(
        map((profile: User) => {
          this.setUserAndToken(this.getJwtToken(), profile, true);
          return profile;
